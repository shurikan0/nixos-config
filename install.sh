...
nix-shell -p git --command "git clone https://github.com/shurikan0/nixos-config.git $SCRIPT_DIR"
...
sed -i "0,/shurikan/s//$(whoami)/" $SCRIPT_DIR/flake.nix
sed -i "0,/Jan/s//$(getent passwd $(whoami) | cut -d ':' -f 5 | cut -d ',' -f 1)/" $SCRIPT_DIR/flake.nix
sed -i "s/jan-gruhnert@startmail.com//" $SCRIPT_DIR/flake.nix
